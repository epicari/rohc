...
static int rohc_release_decomp(struct rohc_init *rcouple) {
	...
	rcouple->decompressor = rohc_decomp_new2(ROHC_SMALL_CID, ROHC_SMALL_CID_MAX, ROHC_U_MODE);
...
/*
	...
	feedback_to_send.max_len = skb->data_len;
...
static int rohc_decomp(struct rohc_init *rcouple, struct sk_buff *skb) {
	...
	uint8_t decomp_buf[r_skb->data_len];
	struct rohc_buf decomp_packet = rohc_buf_init_empty(decomp_buf, r_skb->data_len);
/*
