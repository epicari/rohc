...
static int rohc_release(struct rohc_init *rcouple) {
	...
	//rcouple->rcvd_feedback_buf[BUFFER_SIZE] = NULL;
...
static int rohc_release_decomp(struct rohc_init *rcouple) {
	...
	rcouple->decompressor = rohc_decomp_new2(ROHC_SMALL_CID, ROHC_SMALL_CID_MAX, ROHC_O_MODE);
...
static int rohc_comp(struct rohc_init *rcouple, struct sk_buff *skb) {
	...
	struct rohc_buf ip_packet = rohc_buf_init_full(skb->data, len, arrival_time);

	uint8_t rohc_pkt_buf[len];
	struct rohc_buf rohc_packet = rohc_buf_init_empty(rohc_pkt_buf, len);
...
static int rohc_decomp(struct rohc_init *rcouple, struct sk_buff *skb) {
	...
	uint8_t decomp_buf[len];
	struct rohc_buf decomp_packet = rohc_buf_init_empty(decomp_buf, len);
...
*/	
	...
	feedback_to_send.data -= feedback_to_send.offset;
	feedback_to_send.len = 0;
	...
	if(!rohc_comp_deliver_feedback2(rcouple->compressor, rcvd_feedback)) {
